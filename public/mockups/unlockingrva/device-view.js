(() => {
  ...
  const base = "/mockups/unlockingrva/";
  const concepts = {
    a: { folder: "A", desktop: "Main.dc.html", mobile: "Mobile.dc.html" },
    b: { folder: "B", desktop: "AfterDark.dc.html", mobile: "AfterDarkMobile.dc.html" },
    c: { folder: "C", desktop: "Almanac.dc.html", mobile: "AlmanacMobile.dc.html" },
  };
  ...
  function address(key, layout, forced = false) {
    ...
    const desktopFolder = encodeURIComponent(`${item.folder} · Desktop home-html`);
    const mobileFolder = encodeURIComponent(`${item.folder} · Mobile home-html`);
    return base + desktopFolder + "/" + (layout === "mobile" ? mobileFolder + "/" : "")
      + item[layout] + (forced ? `?view=${layout}` : "");
  }
